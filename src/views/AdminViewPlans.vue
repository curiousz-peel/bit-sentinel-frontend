<script setup>
  ...
  import AdminPlanCard from "../components/card/AdminPlanCard.vue";

  const router = useRouter();

  const plans = ref([]);
  const loadedPlans = ref(false);
  ...
  const removePlan = (id) => {
    plans.value = plans.value.filter((n) => n.id != id);
  };
</script>
...
<template>
  <main v-if="loggedIn">
    <div class="background">
      <Hero></Hero>
      <div v-if="loadedPlans" class="plan-cards">
        <div class="margins">
          <h1 class="plan-cards-section-first">.admin_view_plans</h1>
          <div class="container">
            <ul v-if="loadedPlans">
              <TransitionGroup name="plans">
