<script setup>
  ...
  import AdminTypeCard from "../components/card/AdminTypeCard.vue";
  ...
  const removeType = (id) => {
    types.value = types.value.filter((n) => n.id != id);
  };
</script>
...
<template>
  <main v-if="loggedIn">
    <div class="background">
      <Hero></Hero>
      <div v-if="loadedTypes" class="type-cards">
        <div class="margins">
          <h1 class="type-cards-section-first">.admin_view_types</h1>
          <div class="container">
            <ul v-if="loadedTypes">
              <TransitionGroup name="types">
