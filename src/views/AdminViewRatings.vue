<script setup>
  ...
  import AdminRatingCard from "../components/card/AdminRatingCard.vue";

  const router = useRouter();

  const ratings = ref([]);
  const loadedRatings = ref(false);
  ...
  const removeRating = (id) => {
    ratings.value = ratings.value.filter((n) => n.id != id);
  };
</script>
...
<template>
  <main v-if="loggedIn">
    <div class="background">
      <Hero></Hero>
      <div v-if="loadedRatings" class="rating-cards">
        <div class="margins">
          <h1 class="rating-cards-section-first">.admin_view_ratings</h1>
          <div class="container">
            <ul v-if="loadedRatings">
              <TransitionGroup name="ratings">
