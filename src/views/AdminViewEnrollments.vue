<script setup>
  ...
  import AdminEnrollmentCard from "../components/card/AdminEnrollmentCard.vue";

  const router = useRouter();

  const enrollments = ref([]);
  const loadedEnrollments = ref(false);
  ...
  const removeEnrollment = (id) => {
    enrollments.value = enrollments.value.filter((n) => n.id != id);
  };
</script>
...
<template>
  <main v-if="loggedIn">
    <div class="background">
      <Hero></Hero>
      <div v-if="loadedEnrollments" class="enrollment-cards">
        <div class="margins">
          <h1 class="enrollment-cards-section-first">
            .admin_view_enrollments
          </h1>
          <div class="container">
            <ul v-if="loadedEnrollments">
              <TransitionGroup name="enrollments">
...
<style scoped>
  ...
  .enrollment-cards-section-first {
    padding-bottom: 10px;
    padding-left: 20px;
    font-size: 45px;
  ...
  .enrollments-enter-from {
    opacity: 0;
    transform: scale(0.5);
  }
  .enrollments-enter-to {
    opacity: 1;
    transform: scale(1);
  }
  .enrollments-enter-active {
    transition: all 0.5s ease;
  }
  .enrollments-leave-from {
    opacity: 1;
    transform: scale(1);
  }
  .enrollments-leave-to {
    opacity: 0;
    transform: scale(0.5);
  }
  .enrollments-leave-active {
    transition: all 0.5s ease;
    position: absolute;
  }
  .enrollments-move {
