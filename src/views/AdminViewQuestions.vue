<script setup>
  ...
  import AdminQuestionCard from "../components/card/AdminQuestionCard.vue";

  const router = useRouter();

  const questions = ref([]);
  const loadedQuestions = ref(false);
  ...
  const removeQuestion = (id) => {
    questions.value = questions.value.filter((n) => n.id != id);
  };
</script>
...
<template>
  <main v-if="loggedIn">
    <div class="background">
      <Hero></Hero>
      <div v-if="loadedQuestions" class="question-cards">
        <div class="margins">
          <h1 class="question-cards-section-first">.admin_view_questions</h1>
          <div class="container">
            <ul v-if="loadedQuestions">
